...
import { ShieldCheck, Sparkles, Compass, Target, ArrowRight } from 'lucide-react'

interface ReadinessHeaderBannerProps {
  careerGoal: string
...
export const ReadinessHeaderBanner: React.FC<ReadinessHeaderBannerProps> = ({
  careerGoal,
  readinessScore,
  metSkillsCount,
  totalRequiredSkills,
        ...
        <div className="space-y-3 max-w-2xl">
          ...
          <div className="space-y-1">
            <p className="text-xs font-semibold uppercase tracking-wider text-white/70">
              Technical Competency & Requirement Evaluation
            </p>
            <h1 className="font-heading text-2xl sm:text-3xl font-extrabold text-white tracking-tight leading-tight">
              Readiness for <span className="text-[#D8E8DE] underline decoration-white/30 underline-offset-4">{careerGoal}</span>
            </h1>
          </div>

          <p className="text-xs sm:text-sm text-white/85 leading-relaxed">
            See how your verified skill scores, diagnostic assessments, and portfolio builds compare against the core requirements of your chosen career path.
          </p>
          ...
          <div className="pt-1 flex items-center gap-4 text-xs text-white/80 flex-wrap">
            <span className="bg-white/10 px-2.5 py-1 rounded-md font-medium">
              <strong className="text-white">{metSkillsCount} / {totalRequiredSkills}</strong> Required Skills Met
            </span>
            <span className="bg-white/10 px-2.5 py-1 rounded-md font-medium">
              <strong className="text-white">{activeGapsCount}</strong> Active Skill Gaps
            </span>
            <span className="bg-white/10 px-2.5 py-1 rounded-md font-medium">
              <strong className="text-white">Deterministic</strong> Evaluation Engine
            </span>
          </div>
        </div>

        {/* Donut Progress Ring Box */}
        <div className="flex flex-col items-center md:items-end gap-3 shrink-0 w-full md:w-auto">
          <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20 flex flex-col items-center justify-center text-center w-full md:w-48 space-y-2">
            <span className="text-[11px] font-bold uppercase tracking-wider text-white/70 flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-[#D8E8DE]" />
              Career Readiness Index
            </span>

            <ProgressRing value={readinessScore} label="Ready" variant="forest" size={90} strokeWidth={8} />
