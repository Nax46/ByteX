...
interface ReadinessNextMoveCardProps {
  blockerSkillName?: string
  blockerGap?: number
  blockerImportance?: string
}

export const ReadinessNextMoveCard: React.FC<ReadinessNextMoveCardProps> = ({
  blockerSkillName,
  blockerGap,
  blockerImportance,
}) => {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1.5 max-w-xl">
          <div className="flex items-center gap-2">
            ...
            {blockerImportance && (
              <span className="px-2.5 py-0.5 rounded-full bg-[#FEF3C7] text-[#D97706] text-xs font-semibold border border-[#D97706]/20">
          ...
          <h3 className="font-heading text-xl font-extrabold text-[#171918]">
            {blockerSkillName
              ? `Strengthen ${blockerSkillName} to boost career readiness`
              : 'Advance your daily career roadmap modules'}
          </h3>

          <p className="text-xs sm:text-sm text-[#626763] leading-relaxed">
            {blockerSkillName && blockerGap
              ? `${blockerSkillName} is currently your highest impact readiness gap (-${blockerGap} pts). Closing this gap directly advances your technical readiness index.`
